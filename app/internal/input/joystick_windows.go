...
const (
	...
	ridevPageOnly     = 0x00000020 // RIDEV_PAGEONLY — captures all devices on the usage page
	...
	// axisVirtualBase is the first virtual button number used for relative-axis
	// encoder input. Physical buttons occupy numbers 1-64; axis virtual buttons
	// start at 65 (cap index 0 positive) and increment by 2 per axis per direction.
	axisVirtualBase = 65
)
...
// Fields are read via unsafe at known offsets derived from the SDK struct layout.
...
func (c hidpValueCap) isRange() bool          { return c[12] != 0 }
func (c hidpValueCap) usage() uint16          { return *(*uint16)(unsafe.Pointer(&c[56])) }

var (
	riMu        sync.RWMutex
	riState     = map[uintptr]uint64{}              // hDevice → button bitmask (bit N = button N+1)
	riParsed    = map[uintptr][]byte{}               // hDevice → preparsed HID data
	riValCaps   = map[uintptr][]hidpValueCap{}       // hDevice → relative value caps
	riAxisPrev  = map[uintptr]map[uint16]uint32{}    // hDevice → HID usage → last value
)
...
// readInputMask ORs button bitmasks from all connected HID devices tracked by
// the Raw Input goroutine. Event-based: no polling, no missed pulses.
func readInputMask() uint64 {
	riMu.RLock()
	defer riMu.RUnlock()
	var mask uint64
	for _, bits := range riState {
		mask |= bits
	}
	return mask
}
...
func runRawInputLoop() {
	...
	// RIDEV_PAGEONLY with usUsage=0 captures ALL devices on usage page 0x01
	// (Generic Desktop): joystick, gamepad, steering wheel, multi-axis, button
	// boxes, and any other Generic Desktop peripheral.
	rid := riDevice{
		usUsagePage: 0x01,
		usUsage:     0x00,
...
func handleRawInput(lParam uintptr) {
	...
	// RAWHID fields start immediately after RAWINPUTHEADER (offset 24 on 64-bit).
	off := int(unsafe.Sizeof(riHeader{}))
	if len(buf) < off+8 {
		return
	}
	...
	if dwSizeHid == 0 || dwCount == 0 {
		riMu.Lock()
		riState[hdr.hDevice] = 0
		riMu.Unlock()
		return
	}
	// Process the first report only (dwCount=1 is the common case for HID devices).
	...
	// Button detection (HID usage page 0x09).
	...
	riMu.Lock()
	riState[hdr.hDevice] = mask
	riMu.Unlock()

	// Relative-axis detection for encoders that send value changes rather than
	// button presses. Emits virtual button numbers on axisEventCh.
	if caps := getOrFetchValueCaps(hdr.hDevice, preparsed); len(caps) > 0 {
		handleAxisInput(hdr.hDevice, caps, report, preparsed)
	}
}

// handleAxisInput reads relative HID axis values for the given device and emits
// virtual button numbers (axisVirtualBase + capIndex*2 [±1 for direction]) on
// axisEventCh when a value changes from its previous reading.
func handleAxisInput(hDevice uintptr, caps []hidpValueCap, report, preparsed []byte) {
	...
	for i, cap := range caps {
		...
		status, _, _ := procHidPGetUsageValue.Call(
			uintptr(hidpInput),
			uintptr(cap.usagePage()),
			uintptr(cap.linkCollection()),
			uintptr(cap.usage()),
		...
		if status == hidpStatusSuccess {
			readings = append(readings, reading{cap.usage(), value, i})
		}
	}
...
// NumberInputValueCaps is read from HIDP_CAPS at byte offset 52.
func getOrFetchValueCaps(hDevice uintptr, preparsed []byte) []hidpValueCap {
	...
	var capsBuf [128]byte // oversized buffer; HIDP_CAPS is ~58 bytes
