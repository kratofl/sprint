...
func TestWidgetCatalog(t *testing.T) {
	...
	t.Run("editor palette separates driving car settings race and info widgets", func(t *testing.T) {
		want := map[WidgetType]struct {
			category Category
			label    string
		}{
			WidgetGearSpeed:    {CategoryDriving, "Driving"},
			WidgetGear:         {CategoryDriving, "Driving"},
			WidgetSpeed:        {CategoryDriving, "Driving"},
			WidgetRPM:          {CategoryDriving, "Driving"},
			WidgetTyreTemp:     {CategoryDriving, "Driving"},
			WidgetLapTime:      {CategoryTiming, "Timing"},
			WidgetDelta:        {CategoryTiming, "Timing"},
			WidgetGap:          {CategoryTiming, "Timing"},
			WidgetSector:       {CategoryTiming, "Timing"},
			WidgetLapCounter:   {CategoryTiming, "Timing"},
			WidgetSessionTimer: {CategoryTiming, "Timing"},
			WidgetTC:           {CategoryCarSettings, "Car Settings"},
			WidgetABS:          {CategoryCarSettings, "Car Settings"},
			WidgetBrakeBias:    {CategoryCarSettings, "Car Settings"},
			WidgetEngineMap:    {CategoryCarSettings, "Car Settings"},
			WidgetEnergy:       {CategoryCarSettings, "Car Settings"},
			WidgetFuel:         {CategoryRace, "Race"},
			WidgetPosition:     {CategoryRace, "Race"},
			WidgetIncidents:    {CategoryRace, "Race"},
			WidgetFlags:        {CategoryRace, "Race"},
			WidgetHeader:       {CategoryInfo, "Info"},
			WidgetText:         {CategoryInfo, "Info"},
		}

		for widgetType, expected := range want {
			m, ok := byType[widgetType]
			if !ok {
				t.Fatalf("widget %q missing from catalog", widgetType)
			}
			if m.Category != expected.category {
				t.Errorf("widget %q category = %q, want %q", widgetType, m.Category, expected.category)
			}
			if m.CategoryLabel != expected.label {
				t.Errorf("widget %q category label = %q, want %q", widgetType, m.CategoryLabel, expected.label)
			}
		}
	})
