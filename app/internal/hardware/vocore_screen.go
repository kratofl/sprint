...
// ScanVoCore enumerates connected VoCore M-PRO screens over USB.
// Platform-specific implementations:
//   - vocore_scan_windows.go — SetupDI (no CGO, no libusb)
//   - vocore_scan_usb.go     — gousb/libusb (Linux with CGO)
//   - vocore_scan_stub.go    — fallback for unsupported platforms
func ScanVoCore() ([]VoCoreScreen, error) {
	return scanScreensImpl()
}

// screenFromPID constructs a VoCoreScreen for the given VID/PID and optional
// serial string. Unknown PIDs fall back to a 480×800 default.
func screenFromPID(pid uint16, serial string) VoCoreScreen {
