...
function Input({ className, type, ...props }: React.ComponentProps<"input">) {
  return (
    <input
      ...
      data-slot="input"
      className={cn(
        "h-7 w-full min-w-0 rounded-sm border border-border bg-bg-elevated px-2 py-0.5 text-xs/relaxed text-foreground transition-colors outline-none",
        ...
        "data-[status=neutral]:text-text-muted data-[status=neutral]:focus:border-border-strong data-[status=neutral]:focus:text-foreground",
