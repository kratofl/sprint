...
test('telemetry numeric hero values use Saira tabular typography', () => {
  assert.match(gearDisplaySource, /font-saira/)
  assert.match(gearDisplaySource, /tabular-nums/)
  assert.doesNotMatch(gearDisplaySource, /font-mono|font-display/)
})
...
test('telemetry semantic values use orange green and red tokens only', () => {
  ...
  assert.doesNotMatch(`${deltaBarSource}\n${inputTraceSource}`, /cyan|teal|purple|text-success|text-destructive/i)
})
