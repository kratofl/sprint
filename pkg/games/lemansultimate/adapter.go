...
func (a *Adapter) mapToDTO(t *lmuVehicleTelemetry, s *lmuVehicleScoring, si *lmuScoringInfo) *dto.TelemetryFrame {
	...
	return &dto.TelemetryFrame{
		...
		Car: dto.CarState{
			...
			BrakeBiasRear: toF32(t.MRearBrakeBias),
		},
		...
		Electronics: dto.Electronics{
			...
			ABSMax:      t.MABSMax,
			TCCut:       t.MTCCut,
			TCCutMax:    t.MTCCutMax,
			TCSlip:      t.MTCSlip,
			TCSlipMax:   t.MTCSlipMax,
		...
		Race: dto.RaceState{
			Position:       s.MPlace,
			TotalPositions: uint8(si.MNumVehicles),
			GapAhead:       t.MTimeGapCarAhead,
			GapBehind:      t.MTimeGapCarBehind,
		},
		Energy: dto.EnergyState{
			VirtualEnergy: t.MVirtualEnergy,
			SoC:           t.MSoC,
			RegenPower:    t.MRegen,
			// DeployPower: no direct deploy-power-kW field in LMU shared memory; zero.
		},
		Penalties: dto.Penalties{
			Incidents:       s.MNumPenalties,
			TrackLimitSteps: t.MTrackLimitsSteps,
			PitStops:        s.MNumPitstops,
		},
	}
}
